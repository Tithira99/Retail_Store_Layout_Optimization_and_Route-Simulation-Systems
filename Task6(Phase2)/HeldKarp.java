...
public class HeldKarp {
	...
	public HeldKarp(int[][] matrix, int startingCity, int destinationCity) {
        setMatrix(matrix);
    ...
    public List<Integer> calculateHeldKarp() {
        ...
        PairInteger min; // minimum distance of temporary result
        ...
        for (int i = 1; i < size; i++) {
            dict.put(new PairInteger(1 << i, i), new PairInteger(matrix[0][i], 0));
        }
        ...
        // Gather pairs based on subsets leading to the destination city
        resultTemp = new ArrayList<>();
        for (int i = 1; i < size; i++) {
            resultTemp.add(new PairInteger(
                    dict.get(new PairInteger(bits, i)).first + matrix[i][destinationCity],
                    i));
        }
	...
	public void setStartingCity(int startingCity) {
		this.startingCity = RouteSimulation.constrain(0, this.matrix.length - 1, startingCity);
	}

	public void setDestinationCity(int destinationCity) {
		this.destinationCity = RouteSimulation.constrain(0, this.matrix.length - 1, destinationCity);
	}
